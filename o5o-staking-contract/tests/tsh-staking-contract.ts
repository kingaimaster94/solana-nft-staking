...
import fs from 'fs';
import { TshStaking } from "../target/types/tsh_staking";
...
import DEV_KEY from '/root/alphabets/key.json';
const DEV_ENV = {
  CLUSTER_API: 'https://api.devnet.solana.com',
  PROGRAM_ID: '5ZhFL8Fp8Z3TaDHKvETKCi7t4AuVQEZpQZCSLgvedX2h',
  REWARD_TOKEN: 'B4giMGA7DZSQTniJfYiMvKGg7wPF77T8kj9dAAZfcZtG',
  ADMIN: DEV_KEY
};
...
const filepath = 'target/idl/tsh_staking.json';
...
describe("tsh-staking-contract", () => {
  // Configure the client to use the local cluster.
  let provider = anchor.Provider.env();
  anchor.setProvider(provider);
  //const program = anchor.workspace.DaddyStakingContract as Program<DaddyStakingContract>;
  const program = new anchor.Program(idl, idl.metadata.address, provider) as Program<TshStaking>;
